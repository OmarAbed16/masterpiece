...
const ProfileEdit = ({ onProfileChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate phone number: Must be 10-15 digits (0-9)
    const phoneRegex = /^[0-9]{10,15}$/;
    if (!phoneRegex.test(formData.phone)) {
      Swal.fire({
        icon: "error",
        title: "Validation Error",
        text: "Phone number must be between 10 and 15 digits (0-9).",
      });
      return;
    }

    // Validate about field: Max 500 characters
    if (formData.about && formData.about.length > 500) {
      Swal.fire({
        icon: "error",
        title: "Validation Error",
        text: "About section cannot exceed 500 characters.",
      });
      return;
    }
