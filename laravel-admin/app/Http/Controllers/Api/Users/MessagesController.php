...
use Carbon\Carbon; // Correctly import Carbon
...
class MessagesController extends Controller
{
    public function getUserList($userId)
    {
        ...
        // Map each conversation and ensure user1 always matches the $userId
...
public function realTime(Request $request){
    event(new MessagePusher($request->input('username'),$request->input('message')));

    return [];
}


public function realTime1(Request $request){
    event(new ChatPusher($request->input('ReceiverId'),$request->input('message')));
